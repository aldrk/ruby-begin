...
class Main
  attr_reader :trains, :routes, :stations, :passenger_carriages, :cargo_carriages
  ...
  def start
    loop do
      ...
      when 15
        fill_place_in_carriage
      when 16
        fill_volume_in_carriage
      else
  ...
  def menu_info
    ...
    puts '11 - определить маршрут поезда'
    puts '12 - создать пассажирский вагон'
    puts '13 - создать грузовой вагон'
    puts '14 - посмотреть список вагонов поезда'
    puts '15 - занять место в поезде'
    puts '16 - занять объем в поезде'
  ...
  def remove_station_from_route
    ...
    route = routes.filter { |route| route.name == route_name }
    station = stations.filter { |station| station.name == station_name }
    return unless route[0].include?(station[0])
    route[0].remove_station(station[0])
  end
  ...
  def define_train_route
    ...
    train[0].define_route(route[0])
  end

  def add_carriage_to_train
    puts 'Введите номер поезда и id вагона'
    number = gets.chomp
    id = gets.chomp
    train = trains.filter { |train| train.number == number }
    carriage_check = ->(carriage) { carriage.id == id }
    carriage = train[0].type == :cargo ? cargo_carriages.filter(&carriage_check) : passenger_carriages.filter(&carriage_check)
    train[0].add_carriage(carriage[0])
  end
  ...
  def remove_carriage_from_train
    ...
    carriage_check = ->(carriage) { carriage.id == id }
    train = trains.filter { |train| train.number == number }
    carriage = train[0].carriages.filter(&carriage_check)
    train[0].remove_carriage(carriage[0])
  end
  ...
  def create_passenger_carriage
    puts 'Введите id и количество мест в вагоне'
    id = gets.chomp
    count = gets.chomp.to_i
    carriage = PassengerCarriage.new(id, count)
  ...
  def create_cargo_carriage
    puts 'Введите id и объем вагона'
    id = gets.chomp
    volume = gets.chomp.to_i
    cargo_carriages << CargoCarriage.new(id, volume)
  end
  ...
  def show_train_carriages
    ...
    number = gets.chomp
    train = trains.filter { |train| train.number == number }
    train[0].show_carriages
  end

  def fill_place_in_carriage
    puts 'Введите номер поезда и id вагона и номер места'
    number = gets.chomp
    id = gets.chomp
    place = gets.chomp.to_i
    train = trains.filter { |train| train.number == number }
    carriage_check = ->(carriage) { carriage.id == id }
    carriage = train[0].type == passenger_carriages.filter(&carriage_check)
    carriage[0].fill_place(place)
  end

  def fill_volume_in_carriage
    puts 'Введите номер поезда и id вагона и количество занимаемого объема'
    number = gets.chomp
    id = gets.chomp
    volume = gets.chomp.to_i
    train = trains.filter { |train| train.number == number }
    carriage_check = ->(carriage) { carriage.id == id }
    carriage = train[0].type == cargo_carriages.filter(&carriage_check)
    carriage[0].fill_place(volume)
  end
end
